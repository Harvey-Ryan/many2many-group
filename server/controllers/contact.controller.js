...
const contactController = {
    ...
    createContact: async (req, res) => {
        try {
            ...
            // If opportunity_id(s) are provided, associate the contact with the specified opportunities
            if (req.body.opportunity_ids) {
                const opportunities = await Opportunity.findAll({
                    where: { id: req.body.opportunity_ids },
                });

                if (opportunities.length > 0) {
                    await contact.setOpportunities(opportunities);
                }
            }
    ...
    // Join a contact to an opportunity
    joinContactToOpportunity: async (req, res) => {
        try {
            const contact = await Contact.findOne({
                where: { id: req.body.contact_id },
            });
            const opportunity = await Opportunity.findOne({
                where: { id: req.body.opportunity_id },
            });
            if (!contact) {
                return res.status(404).json({ Status: "Contact not found" });
            }
            if (!opportunity) {
                return res.status(404).json({ Status: "Opportunity not found" });
            }
            await contact.addOpportunity(opportunity);
            return res.json({ Status: "Success", contact });
        } catch (error) {
            console.error("Error occurred while joining contact to opportunity:", error);
            return res.status(500).json({ Status: "Failed to join contact to opportunity" });
        }
    },
